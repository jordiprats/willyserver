package willy.server.sql.processlist;

import java.util.*;

import willy.server.sql.extra.*;
import willy.server.sql.extra.ListSlaves.Slave;
import willy.server.sql.extra.generics.DBStatus;
import willy.server.sql.extra.generics.DBVariables;
import willy.server.sql.extra.grants.DBGrants;
import willy.server.sql.mysql.MySQLTables;
import willy.server.sql.mysql.engines.InnoDBStatus;

//cosa sql
//http://www.dotnetfunda.com/articles/article1949-tricks-to-speed-up-your-sql-query.aspx

//ndb
//mysql> show processlist;
//+----+-------------+-----------+------+---------+------+-----------------------------------+------------------+
//| Id | User        | Host      | db   | Command | Time | State                             | Info             |
//+----+-------------+-----------+------+---------+------+-----------------------------------+------------------+
//|  1 | system user |           |      | Daemon  |    0 | Waiting for event from ndbcluster | NULL             |
//| 12 | root        | localhost | NULL | Query   |    0 | NULL                              | show processlist |
//+----+-------------+-----------+------+---------+------+-----------------------------------+------------------+
//2 rows in set (0.00 sec)



//mysql> show processlist;
//+-----------+-------------+-----------+---------+---------+------+------------------------------+------------------------------------------------------------------------------------------------------+
//| Id        | User        | Host      | db      | Command | Time | State                        | Info                                                                                                 |
//+-----------+-------------+-----------+---------+---------+------+------------------------------+------------------------------------------------------------------------------------------------------+
//| 105020934 | root        | localhost | NULL    | Query   |    0 | NULL                         | show processlist                                                                                     |
//| 105020971 | bbdd_tation | localhost | trendv2 | Query   |    1 | update                       | INSERT INTO `look_new` (`created_at`,`fk_photo`,`sex`,`reason`,`status`) VALUES ('2013-05-24 12:41:0 |
//| 105021001 | bbdd_tation | localhost | trendv2 | Query   |    0 | Waiting for query cache lock | SELECT *  FROM tag_pattern  WHERE tag_pattern.status = '1' AND id_pattern  IN ('4','0' )       ORDER |
//| 105021003 | bbdd_tation | localhost | trendv2 | Query   |    0 | Waiting for query cache lock | SELECT * FROM `user` WHERE id_user = '72811'                                                         |
//| 105021004 | bbdd_tation | localhost | trendv2 | Query   |    0 | Waiting for query cache lock | SELECT photo_tag.*,brand.id_brand brand__id_brand , brand.name brand__name , brand.urlname brand__ur |
//| 105021005 | bbdd_tation | localhost | trendv2 | Query   |    0 | Waiting for query cache lock | SELECT *  FROM art_type  WHERE  1  AND id_type  IN ('28','26','29','31','30','27','70','84','71' )   |
//| 105021006 | bbdd_tation | localhost | trendv2 | Query   |    0 | Waiting for query cache lock | SELECT *  FROM art_type  WHERE  1  AND id_type  IN ('16' )       ORDER BY  FIELD(id_type,'16' )      |
//| 105021007 | bbdd_tation | localhost | trendv2 | Query   |    0 | Waiting for query cache lock | SELECT * ,mk_size.id as mk_size_id FROM mk_size  WHERE mk_size.name = '40'                           |
//| 105021008 | bbdd_tation | localhost | trendv2 | Query   |    0 | Waiting for query cache lock | SELECT * FROM `user` WHERE id_user = '107584'                                                        |
//| 105021009 | bbdd_tation | localhost | trendv2 | Query   |    0 | Waiting for query cache lock | SELECT * ,color_i18n.id as color_i18n_id FROM color_i18n  WHERE color_i18n.body = '' AND color_i18n. |
//+-----------+-------------+-----------+---------+---------+------+------------------------------+------------------------------------------------------------------------------------------------------+
//10 rows in set (0.00 sec)

//while one thread (i.e. one query) is changing anything in the cache, 
//no other query can access it until the lock is released.

//
//lock de innodb:
//	mysql> show processlist;
//	+-----------+-------------+-----------+---------+---------+------+----------------+------------------------------------------------------------------------------------------------------+
//	| Id        | User        | Host      | db      | Command | Time | State          | Info                                                                                                 |
//	+-----------+-------------+-----------+---------+---------+------+----------------+------------------------------------------------------------------------------------------------------+
//	| 105020934 | root        | localhost | trendv2 | Query   |    0 | NULL           | show processlist                                                                                     |
//	| 105038061 | bbdd_tation | localhost | trendv2 | Query   |    1 | Updating       | UPDATE mk_item SET status = '4' WHERE mk_item.status = '7' AND mk_item.sold_at < '2013-05-23 13:05:5 |
//	| 105038062 | bbdd_tation | localhost | trendv2 | Query   |    1 | Updating       | UPDATE mk_item SET status = '4' WHERE mk_item.status = '7' AND mk_item.sold_at < '2013-05-23 13:05:5 |
//	| 105038063 | bbdd_tation | localhost | trendv2 | Query   |    1 | Updating       | UPDATE mk_item SET status = '4' WHERE mk_item.status = '7' AND mk_item.sold_at < '2013-05-23 13:05:5 |
//	| 105038064 | bbdd_tation | localhost | trendv2 | Sleep   |    1 |                | NULL                                                                                                 |
//	| 105038065 | bbdd_tation | localhost | trendv2 | Query   |    1 | Sorting result | SELECT * ,mk_item.id as mk_item_id FROM mk_item  WHERE mk_item.deleted = '0' AND mk_item.sex = '1' A |
//	+-----------+-------------+-----------+---------+---------+------+----------------+------------------------------------------------------------------------------------------------------+
//	6 rows in set (0.00 sec)


public class MySQLWatch extends DBWatch {
	
	/*
	 * Nom�s te en compte l'estat del MySQL
	 */
	
	//TODO: obtindre subconjunts de mostres segons el timestamp
	protected LinkedList<MySQLState> ldbs=null;
	
	/**
	 * llistat de slow queries actualitzades
	 */
	protected LinkedList<DBQuery> ldbq=null;
	
	/**
	 * llista global de slaves
	 */
	protected ListSlaves dbslaves=null;
	
	/**
	 * llista global de slow queries
	 */
	protected ListSlowQueries lslows=null;
	protected boolean hasSlaves=false;
	protected boolean isSlave=false;
	
	//TODO: subconjunts dels vectors de PL i Status
	
	public boolean isSlave() {
		return isSlave;
	}

	public boolean hasSlaves() {
		return this.hasSlaves;
	}

	public MySQLWatch()
	{
		this.ldbs=new LinkedList<MySQLState>();
		this.lslows=new ListSlowQueries();
		this.dbslaves=new ListSlaves();
		
	}
	
	public void setDBItems(MySQLTables dbitems)
	{
		this.ldbs.getLast().setDBItems(dbitems);
		System.out.println("DBITEMS: "+dbitems.toString());
	}
	
	/**
	 * analitza el processlist que tenim extraient:
	 * - genera llistat de slaves vistos (firstseen i lastseen) amb les mostres anteriors
	 */
	public int Do()
	{
		DBProcessList lastdbpl=this.ldbs.getLast().getDBProcessList();
		//proceso l'�ltim processlist:		
		Iterator<DBProcessItem> itdbpi=lastdbpl.iterator();
		
		ldbq=new LinkedList<DBQuery>();
		
		DBQuery dbq=null;
		
		while(itdbpi.hasNext())
		{
			DBProcessItem dbpi=itdbpi.next();

			//TODO: stats
			//http://craig-efrein.developpez.com/articles/jetprofiler-mysql/
			
			/*
			 * TODO: queries locked
+-----------+-----------+---------------------+---------------------+---------+------+----------------------+------------------------------------------------------------------------------------------------------+
| Id        | User      | Host                | db                  | Command | Time | State                | Info                                                                                                 |
+-----------+-----------+---------------------+---------------------+---------+------+----------------------+------------------------------------------------------------------------------------------------------+
| 356267544 | artesanum | 172.25.10.142:50574 | NULL                | Sleep   |    5 |                      | NULL                                                                                                 |
| 356544369 | artesanum | 172.25.10.146:55016 | artesanum           | Query   |  216 | Copying to tmp table | select distinct producto.producto_id from producto join producto_envio
on producto.producto_id=prod |
| 356544440 | artesanum | 10.12.16.9:49742    | artesanum           | Query   |  210 | Locked               | UPDATE producto SET producto_nombre = "" WHERE producto_id = "206287"                                |
| 356544441 | artesanum | 10.12.16.9:49744    | artesanum           | Query   |  210 | Locked               | select usuario_id, producto_id, producto_nombre, producto_ref, producto_desc, producto_precio, produ |
| 356544442 | artesanum | 10.12.16.9:49753    | artesanum           | Query   |  209 | Locked               | select * from producto where producto_id=12522                                                       |
(...)
			 */
//			System.out.println("DEBUG command: "+dbpi.getCommand());
//			System.out.println("DEBUG state: "+dbpi.getState());
			
			//Updating es un possible locked de innodb, verificar amb show innodb status;
			
			//Query executantse, la proceso al llistat de slows
			if(dbpi.getCommand().compareToIgnoreCase(DBWatch.MYSQL_PL_QUERY_STRING)==0)
			{
				//No se que feia aqu� aix�
//				this.hasSlaves=true;
//				lastdbpl.setHasSlave(this.hasSlaves);
				
				dbq=this.lslows.seen(dbpi);
				
				//actualitzo la llista per la GUI
				if(dbq!=null)
					ldbq.add(dbq);
				
				
			}
			
			//si es slave, actualitzo el llistat de slaves que veig
			if(dbpi.getCommand().compareToIgnoreCase(DBWatch.MASTER_SLAVE_SEEN_MYSQL_PL_STRING)==0)
			{
				this.hasSlaves=true;
				lastdbpl.setHasSlave(this.hasSlaves);
				
				System.out.println("SOC MASTER");
				
				//paso el slave vist, actualitza si escau
				dbslaves.seen(dbpi);
			}
			
			
			//despr�s de procesas el Command miro el State, per poder
			if(dbpi.getState()!=null)
			{
				//Waiting for table level lock
				if (
					(dbpi.getState().compareToIgnoreCase(DBWatch.QUERY_LOCKED_WAITING_LOCK_STRING)==0) ||
					(dbpi.getState().compareToIgnoreCase(DBWatch.QUERY_LOCKED_STATE_STRING)==0)
					)
				{
					//indico al item que esta bloquejat
					dbpi.setLocked(true);
					
					//incremento contador de DBQuery que ha estat bloquejada
					if(dbq!=null)
						dbq.seenLocked();
					
					//incremento contador de queries bloquejada del processlist
					lastdbpl.seenLockedQuery();
					
				}

				if(dbpi.getState().compareToIgnoreCase(DBWatch.SLAVE_WAITING_MASTER_MYSQL_PL_STRING)==0)
				{
					this.isSlave=true;

					lastdbpl.setIsSlave(this.isSlave);

					//System.out.println("SOC SLAVE");
				}
			}
			
			//elimino la referencia a DBQuery despr�s de procesar 
			//per seguir amb el seg�ent item
			dbq=null;

		}

		
		/*
		//recorregut de cada processlist analitzant les dades en global
		Iterator<DBProcessList> itdbpl=ldbpl.iterator();
		while(itdbpl.hasNext())
		{
			DBProcessList dbpl=itdbpl.next();
			
			//recorregut per cada process list (variable definida avans com local)
			itdbpi=dbpl.iterator();
			while(itdbpi.hasNext())
			{
				//TODO: fer algo en global
				DBProcessItem dbpi=itdbpi.next();
				
				//tonteria per treure el warning :P
				dbpi.getClass();
			}
			
		}
		*/
		//debug
		//this.lslows.dumpSlowQueries();
		
		return 0;
	}
	
	public void add(DBProcessList dbpl, DBStatus dbs, DBVariables dbv, DBGrants grants, InnoDBStatus idbs)
	{
		this.ldbs.add(new MySQLState(dbpl, (MySQLStatus)dbs, (MySQLVariables)dbv, grants, idbs));
	}
	
	/**
	 * n�mero d'estats del mysql que tenim guardats
	 */
	public int getStateCount()
	{
		//el tamany del llistat d'estats es el n�mero de processlist q tenim
		return ldbs.size();
	}
	
	public MySQLState getLastState()
	{
		return this.ldbs.getLast();
	}
	
	public MySQLState getFirstState()
	{
		return this.ldbs.getFirst();
	}
	
	/**
	 * @deprecated
	 */
	public int getLastProcessListStateCount(String state)
	{
		return this.ldbs.getLast().getDBProcessList().getStateCount(state);
	}
	
	public double[] getGlobalProcessListStateCount(String state)
	{
		double[] a=new double[this.ldbs.size()];
		
		Iterator<MySQLState> it=this.ldbs.iterator();
		
		int i=0;
		while(it.hasNext())
		{
			a[i]=(double)it.next().getDBProcessList().getStateCount(state);
			System.out.println(""+i+"="+a[i]);
			i++;
		}
		
		return a;
	}
	
	
	public int getLastProcessListCount()
	{
		return ldbs.getLast().getDBProcessList().getCount();
	}
	
	public int getSlavesCount()
	{
		return this.dbslaves.getCount();
	}

	public int getActiveSlavesCount() 
	{
		Iterator<Slave> itsl=this.dbslaves.getIterator();
		int activeslaves=0;
		
		while(itsl.hasNext())
		{
			Slave item=itsl.next();
			
			//System.out.println(item.getLastseen()+" vs "+this.ldbpl.lastElement().getTimestamp());
			
			if(item.getLastseen()<this.ldbs.getLast().getDBProcessList().getTimestamp())
				activeslaves++;
		}
		
		return activeslaves;
	}
	
	public int getInactiveSlavesCount()
	{
		return this.dbslaves.getCount()-this.getActiveSlavesCount();
	}
	
	public ListSlaves getListSlaves()
	{
		return this.dbslaves;
	}

	/**
	 * @deprecated
	 * @return cadena ascii de la representaci� del PL
	 */
	public String ProcessListRunningToASCII()
	{
		String out="Threads running * Threads sleep #\n\n";
		
		Iterator<MySQLState> it=this.ldbs.iterator();
		while(it.hasNext())
		{
			DBProcessList dbpl=it.next().getDBProcessList();
			
			//resto 1 pel show full processlist que sempre esta runing
			for(int i=0;i<dbpl.getStateCount(MYSQL_PL_QUERY_STRING)-1;i++)
				out+="*";
			for(int i=0;i<dbpl.getStateCount(MYSQL_PL_SLEEP_STRING);i++)
				out+="#";
			
			out+="\n";
		}
		
		return out;
	}
	
	public String toString()
	{
		return 
				this.ProcessListRunningToASCII()+
				"\nllistat complet queries vistes:"+this.lslows.toString()+
				"\nllistat slows amb maxtime de m�s de 1 segon:"+this.lslows.getSlowQueriesMaxTime(1)+
				"";
	}
	
	public ListSlowQueries getSlowQueriesList()
	{
		return this.lslows;
	}
	
	/**
	 * llistat de slowqueries, TODO: nom desafortunat, canviar
	 */
	public LinkedList<DBQuery> getUpdatedQueries()
	{
		return this.ldbq;
	}

}
